    ...
    >
      ...
      {entries.length === 0 ? (
        ...
        >
          ...
          <p className="text-base font-semibold text-gray-900">No classes scheduled</p>
          <p className="mt-1 max-w-xs text-sm text-gray-500">
            Click Add to create a schedule for this day
          </p>
        </div>
      ) : (
        <ul className="flex max-h-[320px] flex-col gap-2 overflow-y-auto pr-0.5 sm:gap-3">
          {entries.map((e) => (
            <li
              key={e.id}
              className={cn(
