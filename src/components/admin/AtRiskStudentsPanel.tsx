...
export function AtRiskStudentsPanel() {
  ...
  return (
    <div className="rounded-xl border border-red-100 bg-white p-5 shadow-sm">
      <h3 className="mb-3 flex items-center gap-2 text-xs sm:text-sm font-semibold text-red-700">
      ...
      <ul className="space-y-2">
        {students.slice(0, 8).map((s: any) => (
          <li key={s._id} className="flex items-center justify-between rounded-lg bg-red-50 px-3 py-2">
            <span className="text-xs sm:text-sm font-medium text-gray-800">{s.studentId?.fullName || s.studentId?.name}</span>
